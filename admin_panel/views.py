...
from core.captcha_utils import verify_captcha
...
def admin_login(request):
    ...
    if request.method == "POST":
        ...
        is_valid, message = verify_captcha(request.session.session_key, captcha_input)
...
def admin_residents(request):
    ...
    pending_residents = UserProfile.objects.filter(
        role='resident',
        status='pending'
    ).select_related('user')
    ...
    context = {
        ...
        'pending_count': pending_residents.count()
    }

    return render(
        request,
        'admin_panel/adminResidents.html',
        context
    )
...
def approve_resident(request, resident_id):
    ...
    messages.success(
        request,
        f"{resident.full_name} has been approved."
    )

    return redirect('admin_panel:admin_residents')


# REJECT / DELETE RESIDENT
@login_required
def delete_residents(request, resident_id):
    ...
    messages.error(
        request,
        f"{resident.full_name} has been rejected."
    )
...
def admin_notices(request):
    ...
    if request.method == "POST":
        ...
        try:
            send_mail(
                "New Event / Notice Created",

                f"A new notice/event has been created by the admin.\n\n"
                f"Title: {notice.title}\n"
                f"Category: {notice.category}\n\n"
                f"{notice.description}\n\n"
                "Please login to Society Connect to view complete details.",
            ...
            messages.success(request, "Notice created successfully. Emails sent to all users.")
        except Exception as e:
            messages.warning(request, f"Notice created, but emails failed to send: {str(e)}")
...
def update_complaint_status(request, complaint_id):
    ...
    # Send email ONLY when complaint is resolved
        if new_status == 'resolved':
            ...
            try:
                send_mail(
                    ...
                    f"Hello {complaint.user.first_name},\n\n"
                    f"Your complaint regarding '{complaint.category}' has been resolved successfully.\n\n"
                    "Please login to Society Connect for more details.\n\n"
                    "Thank you.",
                ...
                messages.success(request, f"Complaint status updated to {new_status}. Email sent.")
            except Exception as e:
                messages.warning(request, f"Complaint status updated, but email failed to send: {str(e)}")
        else:
...
# CAPTCHA endpoints for admin
@csrf_exempt
def admin_get_captcha(request):
    """Generate and return CAPTCHA image for admin login"""
    if not request.session.session_key:
        request.session.create()
    
    from core.captcha_utils import generate_and_store_captcha
...
@csrf_exempt
def admin_refresh_captcha(request):
    """Refresh CAPTCHA image for admin login"""
    if not request.session.session_key:
        request.session.create()
...
def pending_requests(request):
    ...
    return render(
        request,
        'admin_panel/pending_requests.html',
        {'users': pending_users}
    )
...
def approve_user(request, id):
    ...
    resident_profile = get_object_or_404(
        UserProfile,
        id=id
    )
    ...
    recipient_email = resident_profile.user.email
    ...
    if recipient_email:
        try:
            send_mail(
                ...
                f"Hello {resident_profile.user.first_name},\n\n"
                "Your account has been approved by the admin.\n\n"
                "You can now login to the Society Connect portal using your credentials.\n\n"
                "Thank you.",
            ...
            messages.success(request, "User approved successfully. Email sent.")
        except Exception as e:
            messages.warning(request, f"User approved, but email failed to send: {str(e)}")
    else:
        messages.warning(request, "User approved, but no email address was found for this account.")
    ...
    if recipient_email:
        try:
            send_mail(
                ...
                f"Hello {resident_profile.user.first_name},\n\n"
                "Your account request has been rejected by the admin for security reasons.\n\n"
                "You cannot login to the Society Connect portal.\n\n"
                "Thank you.",
            ...
            messages.error(request, "User rejected successfully. Email sent.")
        except Exception as e:
            messages.warning(request, f"User rejected, but email failed to send: {str(e)}")
    else:
        messages.warning(request, "User rejected, but no email address was found for this account.")
